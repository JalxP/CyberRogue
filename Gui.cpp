...
void Gui::render() {
	...
	int y = 1;
	float colorCoef = 0.4f; // To fade older messages
	for (Message **iterator = log.begin(); iterator != log.end();
		iterator++) {
...
void Gui::renderMouseLook() {
	if (!engine.map->isInFOV(engine.mouse.cx, engine.mouse.cy)) {
		// if mouse is out of fov, nothing no render
		return;
	}
	...
	bool first = true;
	for (Actor **iterator = engine.actors.begin(); iterator != engine.actors.end();
		iterator++) {
...
void Gui::message(const TCODColor &col, const char *text, ...) {
	...
	va_end(ap);
	// So its possible to write several messages in a single Gui::message call
	char *lineBegin = buf;
	char *lineEnd;
